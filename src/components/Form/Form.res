...
@react.component
let make = (~onSubmitClick) => {
  let (formInputs, setFormInputs) = React.useState(_ => initalInputs)
  let {_, transaction_setter, transaction_number, transaction_number_setter} = React.useContext(ContextTxn.TxnContext.context)
